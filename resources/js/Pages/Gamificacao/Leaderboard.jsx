...
export default function Leaderboard({ podio = [], ranking_xp = [], ranking_nivel = [], ranking_badges = [] }) {
        ...
        <AuthenticatedLayout>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    <Link href={route("social.hub")} className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white dark:bg-slate-100 dark:text-slate-900">
                        Ver Rede Social
                    </Link>
                </div>

                <section className={card}>
                    <h2 className="mb-4 text-lg font-bold text-slate-900 dark:text-white">Podio XP</h2>
                    <div className="grid gap-3 md:grid-cols-3">
                        {podioOrdenado.map((u) => (
                            <div key={u.posicao} className="rounded-xl bg-gradient-to-br from-amber-50 to-orange-100 p-4 dark:from-slate-700 dark:to-slate-800">
                                <p className="text-xs font-semibold uppercase text-slate-500">Lugar {u.posicao}</p>
                                <p className="mt-1 text-lg font-extrabold text-slate-900 dark:text-white">{u.usuario?.name}</p>
                                <p className="text-sm text-slate-700 dark:text-slate-300">Nivel {u.nivel} • {u.xp_total} XP</p>
                            </div>
                        ))}
                    </div>
                </section>
