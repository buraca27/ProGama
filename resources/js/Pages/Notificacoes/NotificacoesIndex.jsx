...
export default function Index({ notificacoes, filtros }) {
    const { userRoleReal, notifications: sharedNotifications, prazo_alertas } = usePage().props;
        ...
        return (
            <AuthenticatedLayout>
                ...
                <div className="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <h1 className="text-2xl font-black text-gray-900 dark:text-white mb-2">Acesso Negado</h1>
                    <p className="text-gray-500 dark:text-gray-400 mb-6">
                        A área de notificações não está disponível para a Secretaria.
                    </p>
                    <Link href={route("dashboard")} className="rounded-xl bg-gray-900 px-5 py-2.5 text-sm font-semibold text-white dark:bg-gray-100 dark:text-gray-900">
                        Voltar ao Dashboard
                    </Link>
                </div>
            </AuthenticatedLayout>
    ...
    const tipoAtivo  = filtros?.tipo  ?? null;
    const ordemAtiva = filtros?.ordem ?? "desc";

    const mostraPrazos = tipoAtivo === "Prazo_Proximo";

    const aplicarFiltro = (novoTipo, novaOrdem) => {
        const params = {};
        if (novoTipo)                          params.tipo  = novoTipo;
        if (novaOrdem && novaOrdem !== "desc") params.ordem = novaOrdem;
        router.visit(route("notificacoes.index"), { data: params, preserveScroll: true });
    };
                    ...
                    <>
                        {items.length === 0 ? (
                            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-white py-20 text-center dark:border-gray-700 dark:bg-gray-800">
                                ...
                                <p className="mt-1 text-sm text-gray-400 dark:text-gray-500">
                                    {tipoAtivo ? "Experimenta remover o filtro para veres todas." : "Não tens notificações."}
                                </p>
                            </div>
                        ) : (
                            <div className="space-y-3">
                        ...
                        {ultimaPagina > 1 && (
                            <div className="flex items-center justify-between pt-2">
                                ...
                                <div className="flex gap-2">
                                    {prevHref ? (
                                        <Link href={prevHref} preserveScroll className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">
                                            ← Anterior
                                        </Link>
                                    ) : (
                                        <span className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-semibold text-gray-400 cursor-not-allowed dark:border-gray-700 dark:bg-gray-800 dark:text-gray-600">
                                            ← Anterior
                                        </span>
                                    )}
                                    {nextHref ? (
                                        <Link href={nextHref} preserveScroll className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">
                                            Seguinte →
                                        </Link>
                                    ) : (
                                        <span className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-semibold text-gray-400 cursor-not-allowed dark:border-gray-700 dark:bg-gray-800 dark:text-gray-600">
